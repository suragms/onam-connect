...
import { Copy, Heart, Trash2, Search, RotateCcw } from "lucide-react";
...
export default function SavedPage() {
  ...
  const favoriteGroups = groupHistoryByDate(filteredFavorites);
  const recentGroups = groupHistoryByDate(filteredRecent);

  function renderMessageCard(msg: any) {
    return (
      <div key={msg.id} className="rounded-2xl border border-border/60 bg-card p-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <p className="font-medium">
              🌼 {msg.recipient}{msg.recipientName ? ` — ${msg.recipientName}` : ""}
            </p>
            <p className="text-xs text-muted-foreground">
              {msg.language} · {msg.tone} · {formatRelativeTime(msg.date)}
            </p>
          </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-lg border px-3 py-2.5 text-xs text-destructive cursor-pointer min-h-[44px] flex-1 sm:flex-none"
          >
  ...
  function renderEmptyState(tab: "favorites" | "recent") {
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-4 sm:px-6 py-10">
        <h1 className="text-3xl font-bold tracking-tight">❤️ Saved</h1>
        <p className="mt-2 text-muted-foreground">Your favorites and recent wishes — stored locally on your device.</p>

        <Tabs defaultValue="favorites" className="mt-6">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="favorites" className="min-h-[44px]">
              ❤️ Favorites
            </TabsTrigger>
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className="pl-9 rounded-xl min-h-[48px] text-base"
                />
              </div>
              <select
                value={filter}
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <div className="relative flex-1">
                ...
                <Input
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder="Search recent wishes…"
                  className="pl-9 rounded-xl min-h-[48px] text-base"
                />
              </div>
              <select
